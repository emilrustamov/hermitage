    ...
    @foreach ($products as $product)
        <div class="content-appearance  p-0 product-card" id="content-price" data-sort="{{ $product->price }}">
            ...
            <div class="hover-content">
                ...
                <button class="add-to-cart" data-id="{{ $product->id }}" data-title="{{ $product->title_ru }}" data-price="{{ $product->price }}" data-image="{{ asset($product->image) }}">{{ __('translation.products_trash') }}</button>
            </div>
        </div>
    @endforeach
</div>
